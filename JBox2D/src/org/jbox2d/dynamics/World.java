...
/**
 * The world that physics takes place in. <BR>
 * <BR>
 * To the extent that it is possible, avoid accessing members directly, as in a
 * future version their accessibility may be rolled back - as un-Java as that
 * is, we must follow upstream C++ conventions, and for now everything is public
 * to speed development of Box2d, but it is subject to change. You're warned!
 */
public class World {
	boolean m_lock;
	...
	/**
	 * Get the world body list. With the returned body, use Body.getNext() to
	 * get the next body in the world list. A NULL body indicates the end of the
	 * list.
	 * 
	 ...
	 * 
	 * @param worldAABB
	 *            a bounding box that completely encompasses all your shapes.
	 * @param gravity
	...
	/**
	 * Create a body given a definition. No reference to the definition is
	 * retained. Body will be static unless mass is nonzero. <BR>
	 * <em>Warning</em>: This function is locked during callbacks.
	 */
	 ...
	 * is retained. This function is locked during callbacks. <BR>
	 * <em>Warning</em>: This automatically deletes all associated shapes and
	 * joints. <BR>
	 * <em>Warning</em>: This function is locked during callbacks.
	 */
	...
	/**
	 * Registers a Steppable object to be stepped immediately following the
	 * physics step, once the locks are lifted.
	 * 
	 * @param s
	 */
	...
	/**
	 * Unregisters a method from post-stepping. Fails silently if method is not
	 * found.
	 * 
	 * @param s
	 */
	 ...
	 * up to max count. The number of shapes found is returned.
	 * 
	 * @param aabb
	 *            the query box.
	 * @param maxCount
	...
	public void solve(TimeStep step) {
		...
		for (Body seed = m_bodyList; seed != null; seed = seed.m_next) {
			...
			m_positionIterationCount = Math.max(m_positionIterationCount,
		...
		// Commit shape proxy movements to the broad-phase so that new contacts
		// are created.
	...
	public void solveTOI(TimeStep step) {
		...
		Island island = new Island(m_bodyCount,
				Settings.maxTOIContactsPerIsland, 0, m_contactListener);
		int stackSize = m_bodyCount;
		Body[] stack = new Body[stackSize];
		...
		while (true) {
			...
			for (Contact c = m_contactList; c != null; c = c.m_next) {
				...
				// TODO_ERIN keep a counter on the contact, only respond to M
				// TOIs per contact.
				float toi = 1.0f;
				if ((c.m_flags & Contact.e_toiFlag) != 0) {
					...
					if ((b1.isStatic() || b1.isSleeping())
							&& (b2.isStatic() || b2.isSleeping())) {
						continue;
					}
					...
					toi = TOI.timeOfImpact(c.m_shape1, b1.m_sweep, c.m_shape2,
							b2.m_sweep);
			...
			while (stackCount > 0) {
				...
				for (ContactEdge cn = b.m_contactList; cn != null; cn = cn.next) {
					...
					// Has this contact already been added to an island? Skip
					// slow or non-solid contacts.
					if ((cn.contact.m_flags & (Contact.e_islandFlag
							| Contact.e_slowFlag | Contact.e_nonSolidFlag)) != 0) {
						continue;
					}

					// Is this contact touching? For performance we are not
					...
					// March forward, this can do no harm since this is the min
					// TOI.
			...
			for (int i = 0; i < island.m_bodyCount; ++i) {
				...
				// Invalidate all contact TOIs associated with this body. Some
				// of these
			...
			// Commit shape proxy movements to the broad-phase so that new
	...
	public void drawShape(Shape shape, XForm xf, Color3f color, boolean core) {
		...
		if (shape.getType() == ShapeType.CIRCLE_SHAPE) {
			...
			if (core) {
				m_debugDraw.drawCircle(center, radius - Settings.toiSlop,
						coreColor);
			}
	...
	public void drawDebugData() {
			...
			for (Body b = m_bodyList; b != null; b = b.getNext()) {
				...
				for (Shape s = b.getShapeList(); s != null; s = s.getNext()) {
					if (s.isSensor())
						continue;
					if (b.isStatic()) {
						drawShape(s, xf, new Color3f(255f * 0.5f, 255f * 0.9f,
								255f * 0.5f), core);
					} else if (b.isSleeping()) {
						drawShape(s, xf, new Color3f(255f * 0.5f, 255f * 0.5f,
								255f * 0.9f), core);
					} else {
						drawShape(s, xf, new Color3f(255f * 0.9f, 255f * 0.9f,
								255f * 0.9f), core);
					}
				}
			}
		...
		if ((flags & DebugDraw.e_pairBit) != 0) {
			...
			invQ.set(1.0f / bp.m_quantizationFactor.x,
			...
			for (int i = 0; i < PairManager.TABLE_CAPACITY; ++i) {
				...
				while (index != PairManager.NULL_PAIR) {
					...
					Vec2 x1 = new Vec2(
							0.5f * (b1.lowerBound.x + b1.upperBound.x),
							0.5f * (b1.lowerBound.y + b1.upperBound.y));
					Vec2 x2 = new Vec2(
							0.5f * (b2.lowerBound.x + b2.upperBound.x),
		...
		if ((flags & DebugDraw.e_aabbBit) != 0) {
			...
			for (int i = 0; i < Settings.maxProxies; ++i) {
				...
				AABB b = new AABB();
				b.lowerBound.x = worldLower.x + invQ.x
						* bp.m_bounds[0][p.lowerBounds[0]].value;
				b.lowerBound.y = worldLower.y + invQ.y
						* bp.m_bounds[1][p.lowerBounds[1]].value;
				b.upperBound.x = worldLower.x + invQ.x
						* bp.m_bounds[0][p.upperBounds[0]].value;
				b.upperBound.y = worldLower.y + invQ.y
		...
		m_debugDraw.drawPolygon(vsw, 4, new Color3f(255.0f * 0.3f,
				255.0f * 0.9f, 255.0f * 0.9f));
